	...
	public static List<Table> getAllTables (HWPFDocument doc)
	{
		...
		for(int i=0;i<range.numParagraphs();i++)
		{
			try
			{
				...
				if((par.isInTable()) && (currentlyInTable == false))
				{
					currentlyInTable = true;
						try
						{
							Table table=range.getTable(par);
							AlisaDebug.debug ("WordImport", "table level=" + par.getTableLevel());
...
//
//							if ((reqIdentifier != null) && (reqTitle.length() > 0) && (reqIdentifier.length() > 0))
//							{
//
//								//    	              	            		AlisaDebug.debug ("WordImport" , "rowId  = " + rowId + " identifier " + reqIdentifier + " new requirement title = " + reqTitle);
...
//			}
			            ...
			            {
			            	AlisaDebug.debug ("WordImport", "Processing table" + table);
